...
setup-venv:
	...
	if [ ! -d ".git" ]; then \
		...
		if [ "$(VENV_RELEASE)" = "true" ] || [ "$(VENV_RELEASE)" = "TRUE" ]; then \
			SETUPTOOLS_SCM_PRETEND_VERSION_FOR_DCMI=0.0.1 "$(VENV_BIN)/pip" install -e ".[release]" || { echo "Error: Failed to install dependencies (no Git)."; exit 1; }; \
		else \
			SETUPTOOLS_SCM_PRETEND_VERSION_FOR_DCMI=0.0.1 "$(VENV_BIN)/pip" install -e ".[dev]" || { echo "Error: Failed to install dependencies (no Git)."; exit 1; }; \
		fi; \
	else \
